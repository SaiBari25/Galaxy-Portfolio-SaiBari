...
import { PerspectiveCamera, Quaternion, Vector3, Box3, PlaneGeometry, MeshBasicMaterial, Mesh, CanvasTexture } from "three";
...
export function Room() {
  ...
  // replace any wall texture that contains 'techniz' with a canvas that says 'Levi'
  useEffect(() => {
    if (!roomGLTF) return;

    // replace materials that reference an image with 'techniz' in the src
    roomGLTF.scene.traverse((obj: any) => {
      if (obj.isMesh && obj.material) {
        const mat: any = obj.material;
        const map = mat.map;
        if (map && map.image && typeof map.image.src === 'string' && map.image.src.includes('techniz')) {
          ...
          if (ctx) {
            ...
            ctx.font = '80px Tektur';
            ctx.textAlign = 'center';
            ctx.fillText('Levi', canvas.width / 2, canvas.height / 2 + 30);
          }
    ...
    // fallback: detect wall mesh by name and overlay a plane with 'Levi' text
    const wallObj = roomGLTF.scene.getObjectByName('Wall') || (() => {
      let found: any = null;
      roomGLTF.scene.traverse((o: any) => {
        if (!found && o.isMesh && o.name && o.name.toLowerCase().includes('wall')) found = o;
      });
      return found;
    })();

    if (wallObj) {
      const box = new Box3().setFromObject(wallObj);
      const size = box.getSize(new Vector3());
      const center = box.getCenter(new Vector3());

      const canvas = document.createElement('canvas');
      canvas.width = 1024;
      canvas.height = Math.max(512, Math.round(1024 * (size.y / size.x)));
      const ctx = canvas.getContext('2d');
      if (ctx) {
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.fillStyle = '#000000';
        ctx.font = `${Math.round(canvas.height / 2)}px Tektur`;
        ctx.textAlign = 'center';
        ctx.fillText('Levi', canvas.width / 2, canvas.height / 2 + 20);
      }
      const tex = new CanvasTexture(canvas);
      tex.needsUpdate = true;
      const planeGeom = new PlaneGeometry(size.x * 0.9, size.y * 0.9);
      const planeMat = new MeshBasicMaterial({ map: tex, toneMapped: false });
      const plane = new Mesh(planeGeom, planeMat);
      plane.position.copy(center);
      plane.rotation.copy(wallObj.rotation);
      plane.translateZ(0.01);
      roomGLTF.scene.add(plane);
    }
  }, [roomGLTF]);
